...
function fastXfer(src, dst, srcPath, dstPath, opts, cb) {
  ...
  let mode;
  let fileSize;
  let offset = 0; // resume offset
  let signal;     // for AbortController
  let cancelled = false;
  ...
  if (typeof opts === 'function') {
    ...
    if (opts.signal) signal = opts.signal; // support AbortController
  }
  ...
  function checkCancelled() {
    return cancelled || (signal && signal.aborted);
  }

  if (signal) {
    signal.addEventListener('abort', () => {
      if (!cancelled) onerror(new Error('Transfer cancelled'));
    });
  }
    ...
    function tryStat(err, attrs) {
      ...
      dst.open(dstPath, dstOpenMode, (err, destHandle) => {
        ...
        function onread(err, nb, data, dstpos, datapos, origChunkLen) {
          if (err)
            return onerror(err);
          if (checkCancelled()) return onerror(new Error('Transfer cancelled'));
