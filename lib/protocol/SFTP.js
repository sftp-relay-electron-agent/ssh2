...
function fastXfer(src, dst, srcPath, dstPath, opts, cb) {
  ...
  let mode;
  let fileSize;
  let offset = 0; // NEW: resume offset (bytes)
  ...
  if (typeof opts === 'function') {
    ...
    // NEW: offset support
    if (typeof opts.offset === 'number' && opts.offset >= 0 && !isNaN(opts.offset)) {
      offset = opts.offset;
    }
  }
  ...
  let fsize;
  // pdst = next destination position we will write to; initialize to offset for resume
  let pdst = offset;
  let total = 0; // bytes transferred in THIS run (not including offset)
  let hadError = false;
  let srcHandle;
  let dstHandle;
  let readbuf;
  // buffer size should be based on remaining bytes, not whole file when resuming
  ...
  src.open(srcPath, 'r', (err, sourceHandle) => {
    ...
    function tryStat(err, attrs) {
      ...
      // If offset >= fsize, nothing to do
      if (offset >= fsize) {
        // close source handle then callback success
        src.close(srcHandle, (errClose) => {
          srcHandle = undefined;
          if (errClose)
            return onerror(errClose);
      ...
      // OPEN destination:
      // If resuming (offset > 0) we must NOT truncate the destination:
      // use 'r+' (read/write) to preserve existing data. If starting fresh use 'w'.
      const dstOpenMode = (offset > 0) ? 'r+' : 'w';

      dst.open(dstPath, dstOpenMode, (err, destHandle) => {
        if (err) {
          // If trying to resume but the destination doesn't exist, return an error
          if (offset > 0) {
            return onerror(new Error('Destination does not exist to resume from'));
          }
          return onerror(err);
        }
        ...
        // If remaining <= 0 it's done (shouldn't happen because checked above)
        if (remaining <= 0)
          return cb();

        // Use less memory where possible — base on remaining, not whole fsize
        ...
        // Note: psrc is the offset into readbuf where we write the incoming
        // chunk. Keep it starting from 0 — it is independent of file offset.
        function onread(err, nb, data, dstpos, datapos, origChunkLen) {
          ...
          function writeCb(err) {
            ...
            total += nb;
            // Call onstep with absolute bytes transferred if caller expects it:
            // total + offset = absolute bytes written from start of file.
            onstep && onstep(total + offset, nb, fsize);

            // If read shorter than requested, request the remaining bytes in that chunk
            if (nb < origChunkLen)
              return singleRead(datapos, dstpos + nb, origChunkLen - nb);

            // finished entire file?
            if (total === remaining) {
            ...
            // if we've scheduled reads up to file end, do nothing
            if (pdst >= fsize)
              return;

            // determine next chunk size (watching for file end)
            const chunk =
              (pdst + chunkSize > fsize ? fsize - pdst : chunkSize);
            // schedule next read for chunk starting at pdst
            singleRead(datapos, pdst, chunk);
            pdst += chunk;
          }
        }

        function makeCb(psrc, pdstLocal, chunk) {
          return (err, nb, data) => {
            onread(err, nb, data, pdstLocal, psrc, chunk);
          };
        }

        function singleRead(psrc, pdstLocal, chunk) {
          // src.read(handle, buffer, bufferOffset, length, fileOffset, cb)
          src.read(srcHandle,
                   readbuf,
                   psrc,
                   chunk,
                   pdstLocal,
                   makeCb(psrc, pdstLocal, chunk));
        }

        function startReads() {
          let reads = 0;
          // psrc is buffer offset — start from 0 as usual
          let psrc = 0;
          // pdst already initialized to offset, so first chunk will read from that
